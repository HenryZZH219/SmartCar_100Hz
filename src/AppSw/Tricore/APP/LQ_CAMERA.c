...
/*
            int z;
            unsigned char bin1_Data[60][12];
            for (i = 0; i < 60; i++)
            {
                for (j = 0; j < 12; j=j+1)
                {   if (j==11)
                {bin1_Data[i][j]=Bin_Image[i][8*j]*2*2*2*2*2*2*2+Bin_Image[i][8*j+1]*2*2*2*2*2*2+Bin_Image[i][8*j+2]*2*2*2*2*2+
                Bin_Image[i][8*j+3]*2*2*2*2+Bin_Image[i][8*j+4]*2*2*2+Bin_Image[i][8*j+5]*2*2+1*2+1;}
                    else
                     {bin1_Data[i][j]=Bin_Image[i][8*j]*2*2*2*2*2*2*2+Bin_Image[i][8*j+1]*2*2*2*2*2*2+Bin_Image[i][8*j+2]*2*2*2*2*2+
                     Bin_Image[i][8*j+3]*2*2*2*2+Bin_Image[i][8*j+4]*2*2*2+Bin_Image[i][8*j+5]*2*2+Bin_Image[i][8*j+6]*2+Bin_Image[i][8*j+7];}
                     //int(BIN_Image[i][8*j])*2*2*2*2*2*2*2+int(BIN_Image[i][8*j+1])*2*2*2*2*2*2+int(BIN_Image[i][8*j+2])*2*2*2*2*2 +int(BIN_Image[i][8*j+3])*2*2*2*2+int(BIN_Image[i][8*j+4])*2*2*2+int(BIN_Image[i][8*j+5])*2*2+int(BIN_Image[i][8*j+6])*2+int(BIN_Image[i][8*j+2]);
                }
            }
            ...
            UART_PutChar(UART0, 0xef);  //帧头
                           ...
                           for (i = 0; i < 60; i++)
                           {
                               for (j = 0; j < 12; j=j+1)
                               {
                                   ...
                                   UART_PutChar(UART0, bin1_Data[i][j]); //发送数据
                                   for (z = 0; z< 16580; ++z)
                                   {
                                       __asm("NOP");
                                   }
                ...
                TFTSPI_BinRoad(0, 0, LCDH, LCDW, (unsigned char *)output);
                //Image_process((unsigned char *)output);
                TFTSPI_BinRoad(0, LCDH, LCDH, LCDW, (unsigned char *)output);
...
void process(int a[LCDH][LCDW]){
    int output[LCDH][LCDW];

    int i,j;
    int X,Y,x,y;
    float Hd[3][3]={{5.243018, 0.262373, -81.860225},{2.916073, 3.118138, -148.861097}, {0.066705, 0.004447, -0.080028}};
    for (x=0;x<LCDH;x++)
    {
        for (y=0;y<LCDW;y++){
